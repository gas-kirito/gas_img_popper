...
<script type="text/javascript">
  ...
  function gas_img_popper_load() {
    // trig a spinner
    document.getElementById('gas-img-popper-ul').innerHTML = '<div class="gas-cssload"></div>';
...
//  }
  ...
  if ($rs = safe_query('SELECT id,name,alt,ext,caption,w,h,category,thumbnail,date FROM txp_image WHERE '.$where.' ORDER BY id desc') and $count != 0) {
    ...
    while ($row = nextRow($rs)) {
      ...
      $items .= n. '  <a href="'.$img_path.'/'.$row['id'].$row['ext'].'" onclick="window.open(this.href, \'gas_img_popper_window\', \'width=640, height=480, scrollbars, resizable\'); return false;">';
      // thumbnail
      ...
      $items .= n. '    <span>'.$row['w'].'&times'.$row['h'].'px</span><br />';
