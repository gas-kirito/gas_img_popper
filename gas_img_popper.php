...
//}
function gas_img_popper_article_ui($event, $step, $data, $rs) {
  $_ul = '
  <ul id="gas-img-popper-ul">
  <li>'.gTxt('gas_img_popper_hint').'</li>
  </ul>';
  
  $content = '<div class="txp-container">'.gas_img_popper_category().$_ul.'</div>';
  
  if (is_callable('wrapRegion')) { // new in txp 4.6
    return $data.wrapRegion('gas-img-popper-group', $content , 'gas-img-popper-group-content', 'gas_img_popper_tab_title', 'article-gas-img-popper');
  } else {
    return '
<div id="gas-img-popper-group"><h3 class="plain lever"><a href="#gas-img-popper-link">' . gTxt('gas_img_popper_tab_title') . '</a></h3>
<div id="gas-img-popper-group-content" class="toggle" style="display:none">' .
  $content . '
</div>
</div>
  ';
